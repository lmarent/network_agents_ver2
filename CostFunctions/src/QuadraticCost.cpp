/*! \file   QudraticCost.cpp
    ...
    $Id: QuadraticCost.cpp 748 2016-06-23 15:29:00 amarentes $
*/
...
double ChoiceNet::Eco::getEvaluation( double variable )
{
    ...
    double a = parameters[string("a")];
    double b = parameters[string("b")];
    double c = parameters[string("c")];
    return (a* pow(variable,2)) + (b*variable) + c;
...
ChoiceNet::Eco::defParams_t * ChoiceNet::Eco::getParameters( void )
{
	...
	if (defParameters.size() == 0){
		defParameters.insert( std::pair<std::string, std::string>(std::string("1"), std::string("a")));
		defParameters.insert( std::pair<std::string, std::string>(std::string("2"), std::string("b")));
		defParameters.insert( std::pair<std::string, std::string>(std::string("3"), std::string("c")));		
	}
...
{
	return std::string("QuadraticCost");
}
